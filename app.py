...
def trata_csv(df):
    df['cpf'] = df['cpf'].str.replace('[^0-9]', '')  # remove caracteres não numéricos do cpf
    df['cnpj'] = df['cnpj'].str.replace('[^0-9]', '')  # remove caracteres não numéricos do cnpj
...
def processar_csv():
    ...
    df = pd.read_csv(obj['Body'])  # carrega o arquivo CSV em um dataframe
